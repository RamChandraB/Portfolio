var batteryLevel, winds = {}, memory = {}, _nowapp, fulsapp = false, appsHistory = [], nowwindow, appicns = {}, fileslist = [], badlaunch = false, initmenuload = true, fileTypeAssociations = {}, handlers = {}, Gtodo, notifLog = {}, initialization = false, onstartup = [], novaFeaturedImage = `Dev.png`, portfolioAppsList = [
	"about",
	"experience",
	"projects",
	"skills",
...
const PORTFOLIO_USER = "Ram Chandra";

/*
 * Visitors should land on the desktop, not on an account picker. If a NovaOS
 * account already exists on this device we unlock it silently (the portfolio
 * account is left on the default password on purpose); otherwise we provision
 * one and run the normal first-boot install. A password-protected account is
 * the one case where we still hand over to the real login screen.
 */
async function bootIntoPortfolio(dbResult) {
	let users = [];
	try {
		users = (await sharedStore.getAllUsers()) || [];
	} catch (error) {
		console.error("Could not read the user list:", error);
	}

	const existingUser = users.includes(PORTFOLIO_USER) ? PORTFOLIO_USER : users[0];

	if (existingUser && (dbResult || dbResult == 3)) {
		await cleanupram();
		CurrentUsername = existingUser;

		let unlocked = false;
		try {
			unlocked = await checkPassword("nova");
		} catch (error) {
	...
	await cleanupram();
	CurrentUsername = PORTFOLIO_USER;
	await initializeOS();
}
...
const portfolioUserIcon = "data:image/svg+xml;base64," + btoa(
	`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96 96" width="96" height="96">` +
	`<defs><linearGradient id="pfg" x1="0" y1="0" x2="1" y2="1">` +
	`<stop offset="0" stop-color="#6179FF"/><stop offset=".6" stop-color="#3A48A8"/>` +
	`<stop offset="1" stop-color="#96EA6E"/></linearGradient></defs>` +
	`<rect width="96" height="96" rx="26" fill="url(#pfg)"/>` +
	`<text x="48" y="61" text-anchor="middle" fill="#FFFFFF" font-size="33" font-weight="700" ` +
	`font-family="Poppins, Segoe UI, Arial, sans-serif">RC</text></svg>`
);
...
const portfolioDesktopLayout = [
	...
	{ app: "Shell", label: "Shell" }
];

const portfolioDockLayout = ["About", "Experience", "Projects", "Skills", "Resume", "Contact", "Shell", "Files", "Settings"];

async function pinAppShortcut(folder, appName, label) {
	const app = await getFileByPath(`Apps/${appName}.app`);
	if (!app || !app.id) {
		console.warn(`Skipped pinning ${appName}: the app is not installed.`);
		return false;
	}
	const shortcut = `data:application/json;base64,${btoa(JSON.stringify({ open: app.id }))}`;
	await createFile(folder, `${label || appName}.lnk`, false, shortcut);
	return true;
}

/*
 * First boot lays out the portfolio itself: shortcuts on the desktop, the same
 * apps plus Files and Settings in the dock, and the real PDF dropped into
 * Downloads so it is reachable from the Files app.
 */
async function seedPortfolioWorkspace() {
	try {
		for (const item of portfolioDesktopLayout) {
			await pinAppShortcut("Desktop/", item.app, item.label);
		}
		for (const appName of portfolioDockLayout) {
			await pinAppShortcut("Dock/", appName);
		}
	} catch (error) {
		console.error("Could not lay out the portfolio workspace:", error);
	}

	try {
		const response = await fetch("assets/RamChandra_Beniwal_Resume.pdf");
		if (response.ok) {
			...
			await createFile("Downloads/", "Ram Chandra Beniwal - Resume.pdf", "pdf", dataUri);
		}
	} catch (error) {
		console.error("Could not stage the resume PDF:", error);
	}
}
...
async function initializeOS() {
	...
	setdb().then(async function () {
		...
		await ensureAllSettingsFilesExist()
			...
			.then(async () => {
				await sharedStore.set(CurrentUsername, "icon", portfolioUserIcon);
				await seedPortfolioWorkspace();
				nonotif = false;
				await startup();
				notify(
					"Hi, I'm Ram Chandra.",
					"This whole desktop is my portfolio. Start with About Me, or open Shell and type \"help\".",
					"Portfolio"
				);
	...
	if (appbarelement) {
		try {
			...
			if (Array.isArray(x) && x.length === 0) {
				// Fallback if the dock was never seeded (or the user emptied it).
				const y = await getFileNamesByFolder("Apps");
				const isTouch = 'ontouchstart' in window || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0;
				const fallbackOrder = isTouch
					? ["About.app", "Projects.app", "Resume.app", "Contact.app"]
					: ["About.app", "Experience.app", "Projects.app", "Skills.app", "Resume.app", "Contact.app", "Shell.app", "Files.app", "Settings.app"];
				x = fallbackOrder
					.map(name => y.find(item => item.name === name))
					.filter(Boolean);
			}
			// Sequential so dock icons keep the folder's insertion order.
			for (let app of x) {
				...
				tooltisp.innerHTML = shortcutLabel || basename(app.name);
				appShortcutDiv.appendChild(iconSpan);
...
async function realgenDesktop() {
	...
	try {
		...
		// Sequential so icons keep the folder's insertion order instead of
		// racing each other through getFileById.
		for (let app of y) {
			var appShortcutDiv = document.createElement("div");
			appShortcutDiv.className = "app-shortcut ctxAvail sizableuielement";
			appShortcutDiv.setAttribute("unid", app.id || '');
			...
			if (mtpetxt(app.fileName) == "lnk") {
				let z = JSON.parse(decodeBase64Content(app.content));
				shortcutLabel = basename(app.fileName);
				app = await getFileById(z.open);
			}
			...
			var nameSpan = document.createElement("span");
			nameSpan.className = "appname";
			nameSpan.textContent = shortcutLabel || basename(app.fileName);
			appShortcutDiv.appendChild(iconSpan);
			appShortcutDiv.appendChild(nameSpan);
...
document.addEventListener("DOMContentLoaded", async function () {
	...
	waitForNonNull().then(async (result) => {
		...
		try {
			await bootIntoPortfolio(result);
		} catch (error) {

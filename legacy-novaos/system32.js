...
async function ensureFileExists(fileName = "preferences.json", dirPath = "System/") {
    ...
    // Right after a user switch the memory cache can still answer null. Force
    // one fresh read before deciding there is nothing to write into.
    if (!memory || !memory.root) {
        MemoryTimeCache = null;
        await updateMemoryData();
    }
    if (!memory || !memory.root) return;
    try {
